import { Link } from "react-router-dom";
import classNames from "classnames/bind";
import styles from "./Home.module.scss";
import { useEffect, useState } from "react";
import axios from "axios";

const cx = classNames.bind(styles);

function Home() {
  const [token, setToken] = useState(localStorage.getItem("token"));
  const [showImage1, setShowImage1] = useState(false);
  const [showImage2, setShowImage2] = useState(false);
  const [showImage3, setShowImage3] = useState(false);

  const [profile, setProfile] = useState();
  const [showProfile, setShowProfile] = useState(false);

  const handleLogout = () => {
    const config = {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    };

    axios
      .get(`${process.env.REACT_APP_BACKEND_API}/api/users/sign-out`, config)
      .then((response) => {
        localStorage.removeItem("token");
        setToken(null);
      })
      .catch((error) => {
        console.error(error.message);
        alert("An error occurred while signing innnnn");
      });
  };

  const handleFeaturesClick = (showImage, setShowImage) => {
    setShowImage(!showImage);
    // image ? setImage(false) : setImage(true);
  };

  // const handleLinkSignIn = () => {
  //   const checkToken = localStorage.getItem("token");
  //   if (checkToken) {
  //     setShowImage3(!showImage3);
  //   } else {
  //     window.location.href = "/sign-in";
  //     alert("Bạn cần phải đăng nhập để sử dụng tính năng này");
  //   }
  // };

  const handleShowProfile = (event, type, destiny) => {
    event.preventDefault();
    const token = localStorage.getItem("token");
    const config = {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    };

    axios
      .get(`${process.env.REACT_APP_BACKEND_API}/api/users/posts`, config)
      .then((response) => {
        const data = response.data;

        if (data.status === 1) {
          // setProfile(JSON.stringify(data.user));
          // setProfile(JSON.stringify(data.mess));

          // console.log(JSON.stringify(profile), "profile");
          if (type === "profile") {
            setShowProfile(!showProfile);
          } else if (type === "goLocation") {
            window.location.href = `${destiny}`;
          } else {
            setShowImage3(!showImage3);
          }
          // alert(data.message);
        } else {
          alert(data.message);
          setToken(localStorage.removeItem("token"));
          if (type === "profile") {
          } else {
            window.location.href = "/sign-in";
            alert("Bạn cần phải đăng nhập để sử dụng tính năng này");
          }
        }
      })
      .catch((error) => {
        console.error(error.message);
        alert("An error occurred while signing innnnn");
      });
  };
  useEffect(() => {}, [token]);
  return (
    <div>
      <h1>Đây là trang chủ (Homepage)</h1>
      <div>
        {token ? (
          <button className={cx("logoutButton")} onClick={handleLogout}>
            Đăng xuất
          </button>
        ) : (
          <div>
            <div>
              <Link to="/sign-in">
                <button className={cx("signInButton")}>Đăng nhập</button>
              </Link>
            </div>
            <div>
              <Link to="/sign-up">
                <button className={cx("signInButton")}>Đăng ký</button>
              </Link>
            </div>
          </div>
        )}
      </div>

      {token && (
        <div>
          <div>
            <button
              className={cx("featuresButton")}
              onClick={() => handleFeaturesClick(showImage1, setShowImage1)}
            >
              Tính năng chỉ có thể dùng sau khi đăng nhập (chỉ hiển thị khi đăng
              nhập)
            </button>

            {/* <Link to="/red-feature"> */}
            <button
              className={cx("featuresButton")}
              // className={cx("profile-1")}
              onClick={(event) =>
                handleShowProfile(event, "goLocation", "/red-feature")
              }
            >
              Sang trang này (Red)
            </button>
            {/* <button className={cx("featuresButton")}>
                Sang trang này (Red)
              </button> */}
            {/* </Link> */}
            {showImage1 && (
              <div>
                <h1 className={cx("header-red")}>VD:</h1>

                <ul>
                  <li className={cx("li-red")}>Thay đổi thông tin cá nhân</li>
                  <li className={cx("li-red")}>Công việc đã ứng tuyển</li>
                  <li className={cx("li-red")}>Cài đặt</li>
                </ul>
              </div>
              // <div>
              //   <h1>Tính năng hiển thị dòng chữ này và ảnh:</h1>
              //   <img
              //     className={cx("featuresImage")}
              //     src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmCjqVf1rJwbakOXBlfk7zSiLNHgaI1V9NPg&usqp=CAU"
              //     alt="An example image"
              //   />
              // </div>
            )}
          </div>
          <div>
            <button
              className={cx("profile-check")}
              // onClick={() => handleFeaturesClick(showProfile, setShowProfile)}
              onClick={(event) => handleShowProfile(event, "profile")}
            >
              Check Token (chỉ hiển thị khi đăng nhập) (kiểm tra token hết hạn
              hay chưa)
            </button>

            {showProfile && (
              <div>
                <h1>Token có hiệu lực 15 phút</h1>
              </div>
            )}
          </div>
        </div>
      )}
      <div>
        <button
          className={cx("profile-1")}
          onClick={(event) => handleShowProfile(event, "check")}
        >
          Tính năng chỉ có thể dùng sau khi đăng nhập (hiển thị ở cả lúc đăng
          nhập và chưa đăng nhập)
        </button>
        {/* <Link to="/green-feature"> */}
        <button
          className={cx("profile-1")}
          onClick={(event) =>
            handleShowProfile(event, "goLocation", "/green-feature")
          }
        >
          {" "}
          Sang trang này (Green)
        </button>
        {/* </Link> */}
        {
          showImage3 && (
            <div>
              <h1 className={cx("header-green")}>VD:</h1>
              <ul>
                <li className={cx("li-green")}>Đăng bài tuyển dụng</li>
                <li className={cx("li-green")}>Đặt hàng (tài liệu IT)</li>
              </ul>
            </div>
          )

          // <h1> Chức năng của nút màu xanh lá cây</h1>
        }
      </div>
      <div>
        <button
          className={cx("profile")}
          onClick={() => handleFeaturesClick(showImage2, setShowImage2)}
        >
          Tính năng có thể sử dụng mà không cần đăng nhập (hiển thị ở cả lúc
          đăng nhập và chưa đăng nhập)
        </button>
        <Link to="/blue-feature">
          <button className={cx("profile")}>Sang trang này (Blue)</button>
        </Link>
        {showImage2 && (
          <div>
            <h1 className={cx("header")}>
              Hầu hết nằm trong Header và Footer, VD:
            </h1>
            <ul>
              <li className={cx("li-blue")}>Bộ lọc - filter các bài post</li>
              <li className={cx("li-blue")}>Tìm kiếm theo tên bài post</li>
              <li className={cx("li-blue")}>Xem blog, báo cáo, thống kê,...</li>
            </ul>
          </div>
          // <div>
          //   <h1>Tính năng hiển thị dòng chữ này và ảnh: </h1>
          //   <img
          //     className={cx("featuresImage")}
          //     src="data:image/jpeg;base64,"
          //     alt="An example image"
          //   />
          // </div>
        )}
      </div>
    </div>
  );
}

export default Home;
